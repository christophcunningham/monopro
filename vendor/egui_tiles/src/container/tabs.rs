...
struct ScrollState {
    ...
    /// LOCAL PATCH (monopro): the left edge of each visible tab last frame, in
    /// content coordinates, so an arrow click can land on a tab rather than a third
    /// of a bar.
    pub tab_lefts: Vec<f32>,

    /// LOCAL PATCH (monopro): the arrow's width plus spacing, as last laid out.
    pub button_and_spacing_width: f32,
}
...
impl ScrollState {
    ...
    pub fn update(&mut self, ui: &egui::Ui, arrow_size: Vec2) -> f32 {
        ...
        self.button_and_spacing_width = button_and_spacing_width;
    ...
    /// LOCAL PATCH (monopro): jump to the next tab's left edge in `dir` (+1 right,
    /// -1 left), instead of scrolling a third of the bar.
    ///
    /// `update` shifts `offset` by an arrow's width whenever the left arrow appears or
    /// disappears, to keep content still under the pointer. A snap wants the tab
    /// flush with the visible edge *after* that shift, so it is pre-subtracted here.
    fn snap(&mut self, dir: f32) {
        let eps = 1.0;
        let target = if dir > 0.0 {
            self.tab_lefts
                .iter()
                .copied()
                .find(|&x| x > self.offset + eps)
        } else {
            self.tab_lefts
                .iter()
                .copied()
        ...
        let target = target.unwrap_or(if dir > 0.0 { self.content_size.x } else { 0.0 });
        let will_show_left = target > 0.0;
        let shift = (will_show_left as i32 - self.show_left_arrow as i32) as f32
            * self.button_and_spacing_width;
        self.offset_debt = 0.0;
        self.offset = (target - shift).at_least(0.0);
    }
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            ui.allocate_ui_with_layout(
                ...
                |ui| {
                    ...
                    let mut tab_lefts = Vec::new();
                    let output = scroll_area.show(ui, |ui| {
                        let origin = ui.cursor().left();
                        ...
                        for (i, &child_id) in self.children.iter().enumerate() {
                            ...
                            tab_lefts.push(response.rect.left() - origin);
                    ...
                    scroll_state.tab_lefts = tab_lefts;
                },
            );
